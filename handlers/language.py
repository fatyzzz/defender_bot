...
async def language_selection_handler(
    ...
    if (message.chat.id != config.ALLOWED_CHAT_ID or
            await check_user_passed(pool, message.from_user.id) or
    ...
    user_mention = message.from_user.mention_html() if message.from_user.username else message.from_user.first_name
    ...
    try:
        ...
        logging.info(f"Сообщение о выборе языка отправлено в чат {message.chat.id}, thread_id={thread_id}, текст: {text}")
    ...
    asyncio.create_task(language_selection_timeout(bot, state, message.chat.id, thread_id, message.from_user.id, pool))
...
async def language_selection_timeout(
    ...
    if current_state == UserState.waiting_for_language:  # Проверяем, ждет ли пользователь все еще
...
async def language_callback_handler(
    ...
    # Получаем кликабельное имя пользователя в формате HTML
    user_mention = callback.from_user.mention_html() if callback.from_user.username else callback.from_user.first_name
    ...
    await start_quiz(callback.message, callback.from_user, state, pool, thread_id=thread_id)