...
async def language_selection_handler(
    ...
    try:
        ...
        logging.info(
            f"Language selection sent to chat {message.chat.id} with thread_id={thread_id}"
        )
    except TelegramBadRequest as e:
        logging.warning(
            f"Failed to send language selection with thread_id={thread_id}: {e}"
        )
        lang_msg = await bot.send_message(
            chat_id=message.chat.id,
            text=dialogs["language_selection"],
        ...
        logging.info(
            f"Language selection sent to chat {message.chat.id} without thread_id"
        )
...
async def language_callback_handler(
    ...
    await start_quiz(
        callback.message, callback.from_user, state, pool, user_data["orig_message_id"]
    )
