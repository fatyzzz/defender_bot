...
async def group_message_handler(
    ...
    **kwargs,
    ...
    if (update.old_chat_member.status not in ("left", "kicked") or
        update.new_chat_member.status != "member" or
...
async def quiz_callback_handler(
    ...
    if selected_idx == correct_idx:
        ...
        asyncio.create_task(delete_message(callback.message.bot, callback.message.chat.id, result_msg.message_id, 10))
        ...
        try:
            await ban_user_after_timeout(callback.message.bot, callback.message.chat.id, callback.from_user.id, pool)
            logging.info(f"User {callback.from_user.id} processed by ban_user_after_timeout due to incorrect answer")
        except Exception as e:
            logging.error(f"Failed to process ban_user_after_timeout for user {callback.from_user.id}: {e}")
        for msg_id in user_messages:
            asyncio.create_task(delete_message(callback.message.bot, callback.message.chat.id, msg_id))
...
async def timeout_handler(
    ...
    try:
        ...
        logging.info(f"User {user.id} processed by ban_user_after_timeout due to quiz timeout")
    except Exception as e:
        logging.error(f"Failed to process ban_user_after_timeout for user {user.id}: {e}")
    for msg_id in user_messages:
